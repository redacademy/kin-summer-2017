 ...
 */
  ...
  */
 function kin_athletics_posted_on() {
  ...
  */
 function kin_athletics_posted_by() {
 	$byline = sprintf(
 		esc_html( 'by %s' ),
  ...
  */
 function kin_athletics_comment_count() {
  ...
  */
 function kin_athletics_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && kin_athletics_categorized_blog() ) {
  ...
  * @return bool
  */
 function kin_athletics_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'kin_athletics_categories' ) ) ) {
 		...
 		set_transient( 'kin_athletics_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so kin_athletics_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so kin_athletics_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in kin_athletics_categorized_blog.
  */
 function kin_athletics_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'kin_athletics_categories' );
 }
 add_action( 'edit_category', 'kin_athletics_category_transient_flusher' );
 add_action( 'save_post',     'kin_athletics_category_transient_flusher' );
  ...
  */
 function kin_athletics_comment_list( $comment, $args, $depth ) {
  ...
  */
 function kin_athletics_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
